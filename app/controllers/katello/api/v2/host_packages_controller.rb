  ...
  class Api::V2::HostPackagesController < Api::V2::ApiController
    ...
    def install
      if params[:packages]
        ...
        task     = async_task(::Actions::Katello::Host::Package::Install, @system, packages)
      ...
      if params[:groups]
        ...
        task   = async_task(::Actions::Katello::Host::PackageGroup::Install, @system, groups)
        respond_for_async :resource => task
      end
    end
    ...
    def upgrade
      if params[:packages]
        packages = validate_package_list_format(params[:packages])
        task     = async_task(::Actions::Katello::Host::Package::Update, @system, packages)
        respond_for_async :resource => task
      end
    end
    ...
    param :host_id, :identifier, :required => true, :desc => N_("UUID of the content-host")
    def upgrade_all
      task     = async_task(::Actions::Katello::Host::Package::Update, @system, [])
      respond_for_async :resource => task
    end
    ...
    def remove
      if params[:packages]
        packages = validate_package_list_format(params[:packages])
        task     = async_task(::Actions::Katello::Host::Package::Remove, @system, packages)
        respond_for_async :resource => task
        return
      end
      ...
      if params[:groups]
        groups = extract_group_names(params[:groups])
        task   = async_task(::Actions::Katello::Host::PackageGroup::Remove, @system, groups)
        respond_for_async :resource => task
      end
    end
